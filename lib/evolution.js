...
/**
 * O ID que a Evolution e o WhatsApp mostram vem sem o sufixo (só os dígitos),
 * mas a API exige "@g.us" para entender que o destino é um grupo. Completar
 * aqui evita a configuração errada mais comum.
 */
function normalizarGrupo(valor) {
  ...
  if (digitos.length >= 15) return digitos + '@g.us';
  return limpo;
}
...
async function enviarParaGrupo(mensagem) {
  ...
  const grupo = normalizarGrupo(process.env.EVOLUTION_GROUP_ID);
...
async function diagnostico(fazerTeste) {
  ...
  if (!bruto.EVOLUTION_GROUP_ID) {
    ...
    const grupoUsado = normalizarGrupo(bruto.EVOLUTION_GROUP_ID);
    config.grupoQueSeraUsado = grupoUsado;
    if (!/@g\.us$/.test(grupoUsado)) {
      avisos.push(
        'EVOLUTION_GROUP_ID não parece um ID de grupo. Deve ser o número longo ' +
        'do grupo (18 dígitos), com ou sem @g.us no final.'
      );
    }
  }
...
module.exports = { processarLead, diagnostico, normalizarGrupo, montarMensagem, validar, linkWhatsApp };
